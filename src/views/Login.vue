...
export default {
  ...
  data () { // 选项 / 数据
    return {
      ...
      nusername: 'test', // 新用户注册名
      nemail: 'test@test.com', // 新用户注册邮箱
      npassword: 'test1234', // 新用户注册密码
      npassword2: 'test1234', // 新用户注册重复密码
      login: 0, // 是否已经登录
  ...
  methods: { // 事件处理器
    ...
    gotoHome: function () { // 用户登录
      ...
      if (!that.emailErr && !that.passwordErr) {
        UserLogin(that.email, that.password, function (data) {
          if (data.code === 0) { // 登录成功
    ...
    newRegister: function () { // 注册提交
      ...
      if (!that.nusernameErr && !that.nemailErr && !that.npasswordErr) {
        ...
        getRegister(that.nusername, that.npassword, that.nemail, function (data) {
          console.log(data)
          if (data.code === 0) { // 注册成功
...
.container-login{
  width: 100vw;
  height: 100vh;
  background: center / cover no-repeat url("~@/assets/img/blog_bg.jpg");
}
.loginTitle{
    text-align: center;
    font-size: 26px;
